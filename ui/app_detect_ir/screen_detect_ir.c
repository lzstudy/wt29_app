...
typedef struct _scr_detect_ir_cxt {
    uint8_t prog_slave;                                                         /**@ slave进度 */
    uint16_t time_read_ir;                                                      /**@ 读取IR周期 */
    sliding_filter_t filter;                                                    /**@ 滤波器 */
}scr_detect_ir_cxt_t;


/**************************************************************************************************
 * @brief  : detect_ir生命周期 - 进入
 * @param  : None
 * @return : None
**************************************************************************************************/
...
/**************************************************************************************************
 * @brief  : detect_ir生命周期 - 销毁
 * @param  : None
 * @return : None
**************************************************************************************************/
static void screen_life_destory(void *arg)
{
    scr_detect_ir_cxt_t *cxt = arg;
 ...
 * @return : None
**************************************************************************************************/
static int ir_val_to_prog(uint16_t value)
{
    int precent, prog;

    precent = value * 100 / 3000;
...
static void screen_life_running(void *arg)
{
    ...
    /* 获取IR值 */
    value = get_analog_device_value(ANA_DEV_IR);

    /* 滤波 */
    filter_val = slider_filter_process(&cxt->filter, value);

    /* 获取prog */
    prog = ir_val_to_prog(filter_val);
...
static screen_life_ops_t detect_ir_sl_ops = {
...
static screen_key_event_t detect_ir_sk_ev = {
    .up   = key_up_event,
    .down = key_down_event,
    .ctrl = key_ctrl_event,
};

gui_screen_t screen_detect_ir = {
    .name = "detect_ir",
    .sl_ops = &detect_ir_sl_ops,
    .sk_ev  = &detect_ir_sk_ev,
};
